...
const CategoryScroller = ({ items = DEFAULT_ITEMS }) => {
  return (
    <div className="px-4 pr-0 md:px-0">
      <div className="bg-transparent hide-scrollbar flex gap-2 overflow-x-scroll pr-1 md:items-center md:justify-center md:gap-3 lg:gap-4 xl:gap-5">
        {items.map((it, idx) => (
          <div
            key={idx}
            className="bg-transparent flex shrink-0 flex-col gap-2 text-center"
            role="button"
            tabIndex={0}
            onKeyDown={(e) => {
          ...
          >
            <img
              ...
              loading="eager"
            />
            <div className="block text-xs font-normal capitalize !leading-[14px] text-ssBlack lg:mt-3 xl:text-base select-none md:select-text">
              {it.label}
            </div>
          </div>
        ))}
